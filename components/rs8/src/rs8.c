...
int rs8_encode(uint8_t data[], int info_len, uint8_t parity[], int parity_len)
{
     ...
     * poly[0]: coefficent of "x^(parity - 1)", index form
     *   :
     * poly[pairy - 1]: coefficent of "1", index form
     */
    ...
    m = data[top++]; // most significant coefficient of information polynomial, vector form
...
static int rs8_syndrome(uint8_t data[], int code_len, uint8_t syndrome[], int parity_len)
{
    ...
    for (int i = 0; i < parity_len; i++) {
		...
		if (shortened_len > 0) {
			...
			for (int j = info_len; j < code_len; j++)
			{ // descending order
				t = gf8_add(gf8_mul(t, gf8_alpha(x)), data[j]);
			}
...
int rs8_decode(uint8_t data[], int code_len, int parity_len)
{
    ...
    int errs = 0;
    for (int i = 0; i < code_len - parity_len; i++) { // correct only the informatin part
